...
class(my_numeric)
# for char
typeof(Columbia_University)
class(Columbia_University)
#for logical
typeof(my_logical)
class(my_logical)
```
...
```{r}
Best_university_in_nyc <- "Columbia University"
...
```{r}
my_favorite_nums<- c(12,21,45,4,353)
my_favorite_nums

new_vec_sqrd_my_fav_nums<- my_favorite_nums*my_favorite_nums
print(new_vec_sqrd_my_fav_nums)
print(my_favorite_nums^3)
```
...
df <- data.frame(
  ...
  Difficulty =c(6,9,8,10)# from 0 to 10 , 10 being the most difficuly
  )
df
...
Column Data Types: Tibbles are more tolerant of different types of columns. For example, tibbles allow you to have list-columns – columns that contain lists or data frames themselves, enabling more complex data structures.
...
```{r}
fav_music <- c("Prince", "REM", "Ryan Adams", "BLOF") #1
num_records <- c(2, 7, 5, 1) #2
num_concerts <- c(0, 3, 1, 0) #3
my_music <- data.frame(fav_music, num_concerts, num_records) #4
names(my_music) <- c("artist", "concerts", "records") #4

my_music$records #5
sum(my_music$records) #6
summary(my_music) #7
```
...
```{r}
library("haven")
danish <- read.table("Data/danish.txt", header = TRUE)
